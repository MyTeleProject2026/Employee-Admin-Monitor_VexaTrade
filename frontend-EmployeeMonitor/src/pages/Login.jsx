    ...
    <div className="min-h-screen bg-[#050812] flex items-center justify-center p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a0e1a] p-6 sm:p-8 shadow-2xl">
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4"><div className="h-16 w-16 rounded-full bg-cyan-500/20 flex items-center justify-center"><Shield size={32} className="text-cyan-400" /></div></div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">Will login as: <span className="text-cyan-400">{employeeName || 'YourName'}@VexaTrade</span></p>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value || '')} className="w-full rounded-xl border border-white/10 bg-[#050812] pl-10 pr-12 py-3 text-white outline-none focus:border-cyan-500 transition" placeholder="Enter your password" required />
          ...
          <button type="submit" disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-cyan-500 to-blue-500 py-3 text-sm font-semibold text-black transition hover:opacity-90 disabled:opacity-60">{loading ? 'Logging in...' : 'Login'}</button>
        </form>

        <div className="mt-6 text-center text-xs text-slate-500 border-t border-white/10 pt-4">
          <p>Don't have an account? <Link to="/register" className="text-cyan-400 hover:text-cyan-300 transition">Register here</Link></p>
