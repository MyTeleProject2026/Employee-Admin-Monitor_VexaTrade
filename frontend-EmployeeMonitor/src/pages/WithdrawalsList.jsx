...
export default function WithdrawalsList() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-6">All Withdrawals</h1>
      <div className="rounded-xl border border-white/10 bg-[#0a0e1a] overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.length === 0 ? (
              <tr><td colSpan="6" className="px-4 py-6 text-center text-slate-500">No withdrawals found.</td></tr>
            ) : (
              items.map((wd) => (
                <tr key={wd.id} className="border-b border-white/5 last:border-0 hover:bg-white/5">
                  <td className="px-4 py-3 text-white">{wd.user_id}</td>
                  <td className="px-4 py-3 text-white">{wd.coin || 'USDT'}</td>
                  <td className="px-4 py-3 text-white">{wd.amount}</td>
                  <td className="px-4 py-3 text-slate-300 truncate max-w-[100px]">{wd.address || '—'}</td>
